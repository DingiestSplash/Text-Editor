...
def main():
    ...
    frame_top = ctk.CTkFrame(window, height=50, width= 1000)
    frame_top_left = ctk.CTkFrame(window, height=50, width=150)
    # Buttons        
    new_button = ctk.CTkButton(
        frame_left, text="New", command=lambda: new_button(window, text_edit))
    save_button = ctk.CTkButton(
        frame_left, text="Save", command=lambda: save_file(window, text_edit))
    open_button = ctk.CTkButton(
        frame_left, text="Open", command=lambda: open_file(window, text_edit))
    ...
    window.bind("<Control-o>", lambda x: open_file.open_file(window, text_edit))
